...
| Setting | Behavior |
|---------|----------|
| This PC identity / manage identities | **Advanced → Station & Identities.** One searchable combobox contains Floor Stations and Admin Identities. Selecting an existing identity assigns it locally to this PC. Type a unique name to add it as a **Floor Station** or **Admin Identity**; the new identity is automatically assigned to this PC. Use the selected identity's explicit **Rename** action to change only its display name without changing its stable id or role. New identities require a configured shared floor. Delete and role conversion are intentionally unavailable. |
...
- An intentional **Identity Save** may also seed a missing `floor_settings.json` from this PC's cached floor policy before applying the requested add/rename. This is the recovery path for a configured, reachable shared folder whose floor file is absent. Startup, polling, Teams saves, operator saves, and password changes never recreate the file.
...
- Admin workflow: install the same app on any machine, Advanced → Station & Identities → Browse → Connect (floor password). Use the single identity control to select or add an Admin Identity for that PC, and use Advanced → Teams & Notifications for Teams policy. Peers pick saved changes up on the next clean UI poll / delivery poll.
