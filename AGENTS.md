...
**Current release:** `v0.2.17` keeps automatic and batch processing moving after accuracy failures, returns operator focus to the first failed step, opens the exact failed Excel cell, recovers cleanly from an already-open report workbook, and adds an accelerated floor demo. See `release/v0.2.17.md`. The `v0.2.16` failed-data preservation and `v0.2.15` readiness, burn-in, Changeover, and dynamic identity behavior remain in place. Operator validation: Browse each PC to the same `.PDU_Notifications` folder (never hard-code paths). **Upgrade every floor PC before creating the first new identity** (schema 2 is unsupported on v0.2.14).
...
- Current released pilot is `v0.2.17`; the updater signing key was rotated in `v0.2.10`. Older installs should be updated manually with the current S-drive installer before relying on updater flow; future updater releases should be signed with the same replacement key.
